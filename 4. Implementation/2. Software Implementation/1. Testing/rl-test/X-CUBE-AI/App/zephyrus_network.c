...
#define AI_NET_OBJ_INSTANCE g_zephyrus_network
 
#undef AI_ZEPHYRUS_NETWORK_MODEL_SIGNATURE
#define AI_ZEPHYRUS_NETWORK_MODEL_SIGNATURE     "57b00d90346238f8b78e36822ec88de3"
...
#define AI_TOOLS_DATE_TIME   "Wed Jan  6 15:17:06 2021"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_ZEPHYRUS_NETWORK_N_BATCHES
#define AI_ZEPHYRUS_NETWORK_N_BATCHES         (1)
...
AI_STATIC ai_array input_1_output_array;   /* Array #6 */
...
AI_STATIC ai_tensor input_1_output;   /* Tensor #6 */
...
AI_TENSOR_OBJ_DECLARE(
  ...
  0x0, 0x0,
  AI_SHAPE_INIT(4, 4, 128, 1, 1), AI_STRIDE_INIT(4, 4, 16, 2048, 2048),
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 1024, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_ZEPHYRUS_NETWORK_IN_NUM, &input_1_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_ZEPHYRUS_NETWORK_OUT_NUM, &dense_1_output, &dense_2_output),
  &dense_0_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool zephyrus_network_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, AI_ZEPHYRUS_NETWORK_ACTIVATIONS_ALIGNMENT));
...

  {
    ...
    nl_0_output_array.data = AI_PTR(activations + 512);
    nl_0_output_array.data_start = AI_PTR(activations + 512);
...
AI_DECLARE_STATIC
ai_bool zephyrus_network_configure_weights(
  ai_network* net_ctx, const ai_buffer* weights_buffer)
{
...

  {
    ...
    dense_2_bias_array.data = AI_PTR(weights + 4104);
    dense_2_bias_array.data_start = AI_PTR(weights + 4104);
    dense_2_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_2_weights_array.data = AI_PTR(weights + 3592);
    dense_2_weights_array.data_start = AI_PTR(weights + 3592);
    dense_1_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_1_bias_array.data = AI_PTR(weights + 3584);
    dense_1_bias_array.data_start = AI_PTR(weights + 3584);
    dense_1_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_1_weights_array.data = AI_PTR(weights + 2560);
    dense_1_weights_array.data_start = AI_PTR(weights + 2560);
    dense_0_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_0_bias_array.data = AI_PTR(weights + 2048);
    dense_0_bias_array.data_start = AI_PTR(weights + 2048);
...
AI_API_ENTRY
ai_bool ai_zephyrus_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_ZEPHYRUS_NETWORK_MODEL_NAME,
      .model_signature   = AI_ZEPHYRUS_NETWORK_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_zephyrus_network_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_zephyrus_network_create(
...
AI_API_ENTRY
ai_handle ai_zephyrus_network_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_zephyrus_network_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= zephyrus_network_configure_weights(net_ctx, &params->params);
  ok &= zephyrus_network_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_zephyrus_network_run(
...
AI_API_ENTRY
ai_i32 ai_zephyrus_network_forward(ai_handle network, const ai_buffer* input)
{
