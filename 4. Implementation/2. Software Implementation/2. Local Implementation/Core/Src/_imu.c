...
uint32_t i2c_tx_flag;
uint32_t i2c_rx_flag;
...
uint8_t IMU_Init(I2C_HandleTypeDef* hI2C) {
	...
	i2c_tx_flag = 0;
		...
		HAL_I2C_Mem_Read_IT(hI2C, IMU_ADDR, DEVICE_REG, 1, &check, 1);
		while (!i2c_rx_flag);
		i2c_rx_flag = 0;
    ...
    if (check == IMU_ACK)  // 0x68 will be returned by the sensor if everything goes well
    {
        ...
        Data = 0;
        HAL_StatusTypeDef state1 = HAL_I2C_Mem_Write_IT(hI2C, IMU_ADDR, PWR_MGMT_1_REG, 1, &Data, 1);
				while (!i2c_tx_flag);
				i2c_tx_flag = 0;

        // Set DATA RATE of 1KHz by writing SMPLRT_DIV register
        Data = 0x07;
        HAL_StatusTypeDef state2 = HAL_I2C_Mem_Write_IT(hI2C, IMU_ADDR, SMPLRT_DIV_REG, 1, &Data, 1);
				while (!i2c_tx_flag);
				i2c_tx_flag = 0;
        ...
        Data = 0x00;
        HAL_StatusTypeDef state3 = HAL_I2C_Mem_Write_IT(hI2C, IMU_ADDR, ACCEL_CONFIG_REG, 1, &Data, 1);
				while (!i2c_tx_flag);
				i2c_tx_flag = 0;
        ...
        Data = 0x00;
        HAL_StatusTypeDef state4 = HAL_I2C_Mem_Write_IT(hI2C, IMU_ADDR, GYRO_CONFIG_REG, 1, &Data, 1);
				while (!i2c_tx_flag);
				i2c_tx_flag = 0;
...
void IMU_dataFetch(imu_t *data_struct) {
    ...
    double pitch = RAD_TO_DEG(atan2(-data_struct->Accel_X_RAW, data_struct->Accel_Z_RAW));
